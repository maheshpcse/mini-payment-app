...
- Cross-site authentication: Pages and Railway are different sites, so refresh-token cookies are third-party. See "Known constraint for authentication" in the server's `docs/DEPLOYMENT.md`; a custom domain for both (e.g. `pay.example.com` + `api.example.com`) is the preferred fix before FE-004 ships.
